...
c This file is able to calculate arrival time residuals at different cases:
c only invert locations, only invert velocity structure, and simultaneously
c invert event locations and velocity structure.
c Modified by Haijiang Zhang

	subroutine resstat_tomoDD(log, idata, ndt, unknowns, d, w, idx,
...
c--- more: residual average, rms, and variance:
      ...
      if(ndt.gt.unknowns) then
         ...
         dvar1= dvar1 / 1
         write(*,*)'>>> ndt < unknown parameters'
         write(log,*)'>>> ndt < unknown parameters'
      endif
      ...
      if(abs(dum+999).lt.0.0001) then   ! orginal data
         ...
         write(log,'(a,f7.4)')' weighted variance [s] = ',dvar1/1e6
