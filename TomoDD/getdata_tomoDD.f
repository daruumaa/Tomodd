c--------------------------------------------------------------------------
c  Modified from getdata.f released with hypoDD
c  Now station file also includes station depths. The format of the station file 
c  is: station latitude(in decimal) longitude(in decimal) depth (in km).
c  It accepts two kinds of cross-correlation data format
c  If CC_format=1, CC data format is the default format (hypoDD format)
c  If CC_format=2, CC data format is as follows:
c  EveID1 EveID2 Station Delay-time CC_coef Phase_type
c------------By Haijiang Zhang, hjzhang@geology.wisc.edu--------------------
cz on 8/31/2003, read event type to determine if events are earthquakes,
cz shots, or blasts

c Read in data

	subroutine getdata_tomoDD(
...
c	character	buf2*20		! Input buffer
	...
	character	dattim*25
	doubleprecision	elon(MAXEVE), elat(MAXEVE)
	integer		nerr
	real		del
	real		dist
	real		dt1, dt2
...
c--This may not be necessary, removed by H. Z.
cz      if (ev_dep(i) .lt. 0.01) ev_dep(i) = 1   ! no 0-depth for ttime!!!
...
16    continue
      if (i.gt.MAXEVE) stop'>>> Increase MAXEVE in tomoDD.inc.'
      goto 10
c----------------------------------------------------------
...
58       if (pha.eq.'P' .or. pha.eq.'p') then
            if (iphase.eq.2) goto 50
            dt_idx(i) = 1
            nccp = nccp+1
         elseif (pha.eq.'S' .or. pha.eq.'s') then
...
c	skip event pairs with large separation distance: 
     ...
     &            (dlon*(cos(ev_lat(iicusp(k1))*PI/180)*111))**2 +
     &            (ev_dep(iicusp(k1))-ev_dep(iicusp(k2)))**2)
            if(maxsep_cc.gt.0 .and. offs.gt.maxsep_cc) skip= 1
            goto 51
         else
c           New format, body...
            if (skip.eq.1) goto 51
c            read (line,*,err=1051) dt_sta(i), dt_dt(i), dt_qual(i), pha
	    read (line,*,err=1051) dtsta, dtdt, dtqual, pha
            if(dtqual.ge.0.and.dtqual.le.20) then
	       dt_sta(i)=dtsta
	       dt_qual(i)=dtqual
               dt_dt(i) = dtdt - otc
               dt_c1(i) = ic1
...
c--Only accept P or S phase codes
59       if (pha.eq.'P'.or.pha.eq.'p') then
...
c	skip event pairs with large separation distance: 
     ...
     &            (dlon*(cos(ev_lat(iicusp(k1))*PI/180)*111))**2 +
     &            (ev_dep(iicusp(k1))-ev_dep(iicusp(k2)))**2)
            if(maxsep_ct.gt.0 .and. offs.gt.maxsep_ct) skip= 1
            goto 90
         else
            if (skip.eq.1) goto 90
...
c       read absolute times (added by H. Z.)      
        if ( trimlen(fn_abs).gt.1 .and. absolute_use.eq.1) then
         call freeunit(iunit)
...
160   ndt = nccp+nccs+nctp+ncts
      ...
      if (ndt.gt.MAXDATA) stop'>>> Increase MAXDATA in tomoDD.inc.'
      if (ndt.eq.0) stop
...
1091   write(*,*)'>>> Format error in catalog data file.'
      ...
      end  ! of subroutine getdata_tomoDD
