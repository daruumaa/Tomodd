C--- reweighting different typies of data (CC data, differential catalog
C--- data, absoute catalog data)
...
c--- allow the weighting curve to decrease slowly.
	      if(maxres_cross.gt.0.and.dt_wt(i).gt.0.000001)
     & dt_wt(i)= dt_wt(i) * (1- (abs(dt_res(i))/maxres_cc)**7)**1
...
c plot(abs(s),ss);  axis([0 0.11 -0.1 1]);
		 ...
		 if(dt_c1(i) .eq. dt_c2(i)) then 
		    dt_wt(i)= dt_wt(i) * (1- (abs(dt_res(i))/maxres_cat)**7)**1
		 else
		    dt_wt(i)= dt_wt(i) * (1- (abs(dt_res(i))/maxres_ct)**7)**1
		 endif
