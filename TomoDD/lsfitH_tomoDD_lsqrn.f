c-----------------------------------------------------------------
c This file is to set up doube-difference tomography equations and 
c solve them by using LSQR method. Only event locations 
c is updated simultaneously. 
c Some newly parameters for the purpose of DD tomograph is as follows:
c tmp_xp, tmp_yp, tmp_zp
c       -------- Partial derivatives with regard to X, Y, and Z (P-wave)
c tmp_xs, tmp_ys, tmp_zs
c       -------- Partial derivatives with regard to X, Y, and Z (S-wave)
c For questions, contact Haijiang Zhang at hjzhang@geology.wisc.edu
c-----------------------------------------------
cz 8/31/2003, add src_type to deal with different data types

	subroutine lsfitH_tomoDD_lsqr(log, iter, ndt, nev, nsrc,
     &	damp,eve_sta,
...
C relocate events if at least one is earthquake  
	   if(src_type(k1).eq.0 .or. src_type(k2).eq.0) then !Earthquake data
	      j=j+1
	   endif
	enddo
	
	nndt=j
	write(*,*)'ndt=',ndt,'nndt=',nndt
...
c       Set up non-zero G matrix elements and apply weights
	   ...
	   if(src_type(dt_ic1(i)).eq.0 .or. 
     &        src_type(dt_ic2(i)).eq.0) then ! earthquake data
	      j=j+1
...
c-- write the derivative matrix terms
c        write(16,*)'i=',i,(rw(k*nndt+j),k=0,7)
